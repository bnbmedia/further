...
```
{
  ...
  hasReceivedNotification: Bool // A boolean variable to be used if this UUID reports a wellness concern
}
```
...
- Whether you are feeling sick and exhibiting symptoms you *think* may be related to a wellness issue
- Whether you have visited a professional for this wellness issue
- The results of your visit to a professional for this issue
...
Each daily update provides a visual representation of the "risk level," ranging from low to high.  A low risk level indicates **no** exposure to any users self-reporting a wellness issue.  A medium risk level indicates **at least one** exposures to a user self-reporting a wellness issue.  A high risk level indicaites **two or more** exposures to users self-reporting a wellness issue.  The results of the daily query are saved locally to the device (and/or in iCloud, if the device is signed into an iCloud account).
...
As noted above, a call to this endpoint (or visiting this URL in a browser) will return a JSON array of Questionnaire models.  This data is updated every 5 minutes, providing only responses where a user has self-reported a visit to a wellness professional and the outcome of that visit leads to concern for others in close proximity.
